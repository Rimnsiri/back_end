...
function updateButtonLabel() {
    ...
    if (selectedSkills.length > 0) {
        ...
        selectedSkills.forEach((skill) => {
            ...
            skillBadge.className = "bg-white badge text-dark me-2"; // Classes Bootstrap pour style
...
document.addEventListener("DOMContentLoaded", function () {
    initializeQuillEditor(0);
    console.log("DOM fully loaded and parsed");

    let groupCounters = {
        "education-form-group": document.querySelectorAll(
            ".education-form-group"
        ).length,
        "experience-form-group": document.querySelectorAll(
            ".experience-form-group"
        ).length,
        "skill-form-group":
            document.querySelectorAll(".skill-form-group").length,
    };
    ...
    function addFormGroup(containerId, className) {
        ...
        if (!formGroup.querySelector(".remove-" + className)) {
            ...
            removeButton.textContent =
                "supprimé " + className.replace("-form-group", "");
        ...
        initializeQuillEditor(experienceIndex);
        
    }
    function initializeQuillEditor(experienceIndex) {
        const editorContainer = document.getElementById(`quill-editor-experience-${experienceIndex}`);
        const hiddenInput = document.getElementById(`quill-input-experience-${experienceIndex}`);
